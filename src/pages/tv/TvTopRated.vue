<template>
    <main class="p-4">
        <h1 class="text-2xl font-bold mb-4">Top Rated TV Series</h1>
...
const fetch = (p = 1) => tvPopular.fetchTopRated(p);
...
function doSearch(p = 1) {
    ...
    if (!term) tvPopular.fetchTopRated(1);
    else tvPopular.findSeries(term, p);      
}

function clearSearch() {
    query.value = '';
    tvPopular.fetchTopRated(1);
}

function go(p: number) {
    if (isSearching.value) doSearch(p);
    else tvPopular.fetchTopRated(p);
}
</script>
